...
class MockBlipTVDepends:
    def mock(self):
        ...
        sys.modules["__main__"].plugin = "BlipTV - IntegrationTest"
    ...
    def log(self, description, level=0):
        if sys.modules["__main__"].dbg and sys.modules["__main__"].dbglevel > level:
            try:
                print "%s [%s] %s : '%s'" % (time.strftime("%H:%M:%S"), sys.modules["__main__"].plugin, inspect.stack()[3][3], description.decode("utf-8", "ignore"))  # 3 - 3 for TestYouTubeUserFeeds.py
            except:
                print "%s [%s] %s : '%s'" % (time.strftime("%H:%M:%S"), sys.modules["__main__"].plugin, inspect.stack()[3][3], description)  # 3 - 3 for TestYouTubeUserFeeds.py
