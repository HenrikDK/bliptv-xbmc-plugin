...
function addButtonWithMenu() {
    var menu = document.getElementById("EmbedTarget");
/*
    var menu = document.getElementsByClassName("EpisodeEndcap");
    if ( menu.length > 0 ) {
	alert(menu_old);
	alert(menu);
	menu = menu[0];
...
function run() {
    if ( window.location.host.indexOf("blip.tv") > -1 ) {
	if ( xbmc_autoplay ) {
	    playVideo(false);
	}
